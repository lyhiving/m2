<div class="comment">
	...
	<ol class="comment-list clearfix">
		...
		<li class="item <?php if ($comment->comment_author_email == get_the_author_email()) echo 'admin '; ?>clearfix"
		    id="comment-<?php comment_ID(); ?>">
			<div class="gravatar"><?php if (get_comment_type() == 'comment') echo get_avatar($comment, 32); else echo '?'; ?></div>
			<dl>
				<dt>
					<em><?php comment_author_link(); ?></em> <?php if ($comment->comment_parent) echo '对 <em><a href="#comment-' . $comment->comment_parent . '" title="查看被回应楼层">' . get_comment($comment->comment_parent)->comment_author . '</a></em> 说'; ?>
					<s>/</s><?php comment_date('Y-m-d H:i'); ?></dt>
